...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    kidsClient.statistics
      .teacher(teacherId)
      .then((s) => {
        if (!cancelled) setTeacherStats(s);
      })
      .finally(() => {
    ...
    try {
      ...
      const msg =
        typeof raw === 'string'
          ? raw
          : Array.isArray(raw)
            ? raw[0] ?? 'Не удалось сохранить профиль'
    ...
    try {
      ...
      const msg =
        typeof raw === 'string'
          ? raw
          : Array.isArray(raw)
            ? raw[0] ?? 'Не удалось сменить пароль'
    ...
    <div className="mx-auto max-w-3xl space-y-8 px-4 py-8">
      ...
      <Card>
        ...
        <p className="mb-2 text-gray-700">
          {telegramLinked
            ? 'Telegram привязан к аккаунту — можно входить через бота или с сайта.'
        ...
        {telegramLinked && (
          <Button
            ...
            onClick={async () => {
              if (
                !confirm(
                  'Отвязать Telegram от аккаунта? Вход через бота и с сайта станет недоступен до повторной привязки.'
                )
              )
                return;
        ...
        {!inTelegram && !telegramLinked && (
          <p className="text-sm text-gray-600">
            Откройте приложение из Telegram-бота и нажмите «Привязать Telegram» здесь, или войдите
            на сайте и привяжите в мини-приложении бота.
          </p>
        )}
      </Card>
      ...
      {role === 'teacher' && teacherStats && (
        <section>
          <h2 className="mb-4 text-xl font-bold text-primary">Мои группы (Focus Kids)</h2>
      ...
      {(!role ||
        ...
        (role === 'teacher' && teacherId != null && !teacherStats && !loading)) && (
        <Card>
