...
import type { Meditation } from '@/types/sense';

export default function SenseMeditationsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <h1 className="text-heading text-sense">Медитации</h1>
        <p className="mt-2 text-gray-700">
          Аудиомедитации для расслабления и развития эмоционального интеллекта.
        </p>
      </section>

      {items.length === 0 ? (
        <Card variant="sense">
          <p className="text-gray-600">Пока нет медитаций. Они появятся после загрузки модератором.</p>
        </Card>
      ) : (
        <div className="space-y-4">
          {items.map((m) => (
            <AudioPlayerSense
              key={m.id}
              audioPath={`meditations/${m.id}/audio`}
              title={m.title}
            />
          ))}
        </div>
